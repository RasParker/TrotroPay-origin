...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Passenger boarding endpoint
  app.post("/api/vehicles/:vehicleId/board", async (req, res) => {
    try {
      if (!req.session.userId) {
        return res.status(401).json({ message: "Not authenticated" });
      }

      const { passengerCount = 1 } = req.body;
      const vehicleId = parseInt(req.params.vehicleId);
      
      const capacityUpdate = await storage.updateVehicleCapacity(vehicleId, passengerCount);
      
      // Broadcast capacity update to vehicle crew
      const capacityNotification = {
        type: "capacity_update",
        vehicleId,
        currentPassengers: capacityUpdate.vehicle.currentPassengers,
        maxCapacity: capacityUpdate.vehicle.maxCapacity,
        capacityPercentage: capacityUpdate.capacityPercentage,
        isFull: capacityUpdate.isFull,
        isNearlyFull: capacityUpdate.isNearlyFull,
        action: "board",
        passengerCount
      };

      broadcastToVehicleCrew(vehicleId, capacityNotification);

      // Send specific notification if vehicle is full or nearly full
      if (capacityUpdate.isFull) {
        broadcastToVehicleCrew(vehicleId, {
          type: "vehicle_full",
          message: "🚫 Vehicle is now FULL! No more passengers can board.",
          currentPassengers: capacityUpdate.vehicle.currentPassengers,
          maxCapacity: capacityUpdate.vehicle.maxCapacity
        });
      } else if (capacityUpdate.isNearlyFull && !capacityUpdate.isFull) {
        broadcastToVehicleCrew(vehicleId, {
          type: "vehicle_nearly_full",
          message: `⚠️ Vehicle is ${capacityUpdate.capacityPercentage}% full. Only ${capacityUpdate.vehicle.maxCapacity - capacityUpdate.vehicle.currentPassengers} seats left!`,
          currentPassengers: capacityUpdate.vehicle.currentPassengers,
          maxCapacity: capacityUpdate.vehicle.maxCapacity
        });
      }

      res.json(capacityUpdate);
    } catch (error) {
  ...
  // Passenger alighting endpoint
  app.post("/api/vehicles/:vehicleId/alight", async (req, res) => {
    try {
      if (!req.session.userId) {
        return res.status(401).json({ message: "Not authenticated" });
      }
